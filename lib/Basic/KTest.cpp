...
KTest *kTest_fromFile(const char *path) {
  ...
  for (i = 0; i < res->numObjects; i++) {
    ...
    if (version >= 4) {
      if (!read_uint32(f, &o->content.numPointers))
        goto error;
      o->content.pointers = (Pointer *)calloc(o->content.numPointers,
                                              sizeof(*o->content.pointers));
      if (!o->content.pointers)
        goto error;
      for (j = 0; j < o->content.numPointers; j++) {
        Pointer *p = &o->content.pointers[j];
    ...
    if (res->objects) {
      for (i = 0; i < res->numObjects; i++) {
        ...
        if (bo->name)
          free(bo->name);
        if (bo->content.bytes)
          free(bo->content.bytes);
        if (bo->content.finalBytes)
  ...
  for (i = 0; i < bo->numObjects; i++) {
    ...
    if (o->content.finalBytes) {
      if (fwrite("Y", 1, 1, f) != 1)
        goto error;
      if (o->content.numBytes &&
          fwrite(o->content.finalBytes, o->content.numBytes, 1, f) != 1)
        goto error;
    } else {
      if (fwrite("N", 1, 1, f) != 1)
        goto error;
    ...
    if (!write_uint32(f, o->content.numPointers))
      goto error;
    for (j = 0; j < o->content.numPointers; j++) {
      Pointer *p = &o->content.pointers[j];
...
unsigned kTest_numBytes(KTest *bo) {
  unsigned i, res = 0;
  for (i = 0; i < bo->numObjects; i++)
    res += bo->objects[i].content.numBytes;
  return res;
}
...
void kTest_free(KTest *bo) {
  ...
  for (i = 0; i < bo->numObjects; i++) {
    ...
    free(bo->objects[i].content.bytes);
    if (bo->objects[i].content.finalBytes) {
