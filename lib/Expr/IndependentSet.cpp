...
getAllIndependentConstraintsSets(const Query &query) {
  ...
  for (auto symcrete : query.constraints.symcretes()) {
    auto iset = IndependentElementSet(symcrete.symcretized);
    iset.exprs.clear();
    auto related = symcrete.marker->source->getRelated();
    for (auto i : related) {
      iset.wholeObjects.insert(i);
    }
    factors->push_back(iset);
  }
...
getIndependentConstraints(const Query &query, std::vector<ref<Expr>> &result) {
  ...
  for (auto symcrete : query.constraints.symcretes()) {
    ...
    worklist.push_back({ref<Expr>(), iset});
  }
