...
class DefaultExprBuilder : public ExprBuilder {
  ...
  virtual ref<Expr> Select(const ref<Expr> &Cond, const ref<Expr> &LHS,
                           const ref<Expr> &RHS) {
  ...
  virtual ref<Expr> Extract(const ref<Expr> &LHS, unsigned Offset,
                            Expr::Width W) {
  ...
  virtual ref<Expr> Not(const ref<Expr> &LHS) { return NotExpr::alloc(LHS); }
...
public:
  ...
  ref<Expr> Constant(const llvm::APInt &Value) { return Base->Constant(Value); }
  ...
  ref<Expr> Select(const ref<Expr> &Cond, const ref<Expr> &LHS,
                   const ref<Expr> &RHS) {
...
public:
  ...
  ref<Expr> Add(const ref<NonConstantExpr> &LHS,
    ...
    case Expr::Add: {
      ...
      // (X + Y) + Z ==> X + (Y + Z)
      return Builder->Add(BE->left, Builder->Add(BE->right, RHS));
    }

    case Expr::Sub: {
      BinaryExpr *BE = cast<BinaryExpr>(LHS);
      // (X - Y) + Z ==> X + (Z - Y)
      return Builder->Add(BE->left, Builder->Sub(RHS, BE->right));
    }
    }
