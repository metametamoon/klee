...
public:
  ...
  virtual void update(const pobs_ty &addedPobs, const pobs_ty &removedPobs) = 0;
...
public:
  explicit InterleavedBackwardSearcher(
      const std::vector<BackwardSearcher *> &searchers);
