...
using ConstantEitherConstantPointer =
    either<klee::ConstantExpr, ConstantPointerExpr>;
...
private:
  ...
  std::map<const llvm::GlobalValue *, ref<ConstantPointerExpr>> globalAddresses;
  ...
  bool lazyInitializeObject(ExecutionState &state, ref<PointerExpr> address,
  ...
  ref<Expr> toExpr(ref<ConstantEitherConstantPointer> cecp);
  ...
  ref<Expr> evalExpr(const llvm::Constant *c, llvm::APFloat::roundingMode rm,
                     const KInstruction *ki = NULL);
  ...
  evalConstant(const llvm::Constant *c, llvm::APFloat::roundingMode rm,
               const KInstruction *ki = NULL);
