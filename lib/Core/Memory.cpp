...
const UpdateList &CachedUList::getUpdates() const {
 ...
 if (auto sizeExpr = dyn_cast<ConstantExpr>(size)) {
    ...
    if (knownSymbolics.storage().size() == size) {
      ...
      auto array = arrayCache->CreateArray(
  ...
  if (!updates.root) {
    SparseStorage<ref<ConstantExpr>> values(
        ConstantExpr::create(0, Expr::Int8));
    auto array = arrayCache->CreateArray(size, SourceBuilder::constant(values));
...
void CachedUList::flushForRead() const {
...
ref<Expr> CachedUList::read(ref<Expr> offset, Expr::Width width) const {
...
ref<Expr> CachedUList::read(unsigned offset, Expr::Width width) const {
  // Treat bool specially, it is the only non-byte sized write we allow.
  if (width == Expr::Bool)
    return ExtractExpr::create(read8(offset), 0, Expr::Bool);
...
ref<Expr> CachedUList::read8(unsigned offset) const {
...
void CachedUList::write(unsigned offset, ref<Expr> value) {
...
void CachedUList::write(ref<Expr> offset, ref<Expr> value) {
  // Truncate offset to 32-bits.
  offset = ZExtExpr::create(offset, Expr::Int32);
...
void CachedUList::write8(unsigned offset, uint8_t value) {
  knownSymbolics.store(offset, ConstantExpr::create(value, Expr::Int8));
  unflushedMask.store(offset, true);
}

void CachedUList::write16(unsigned offset, uint16_t value) {
  unsigned NumBytes = 2;
  for (unsigned i = 0; i != NumBytes; ++i) {
    unsigned idx = Context::get().isLittleEndian() ? i : (NumBytes - i - 1);
...
void CachedUList::write32(unsigned offset, uint32_t value) {
  unsigned NumBytes = 4;
  for (unsigned i = 0; i != NumBytes; ++i) {
    unsigned idx = Context::get().isLittleEndian() ? i : (NumBytes - i - 1);
    write8(offset + idx, (uint8_t)(value >> (8 * i)));
...
void CachedUList::write64(unsigned offset, uint64_t value) {
  unsigned NumBytes = 8;
  for (unsigned i = 0; i != NumBytes; ++i) {
    unsigned idx = Context::get().isLittleEndian() ? i : (NumBytes - i - 1);
    write8(offset + idx, (uint8_t)(value >> (8 * i)));
...
ref<Expr> CachedUList::read8(ref<Expr> offset) const {
...
void CachedUList::write8(unsigned offset, ref<Expr> value) {
...
void CachedUList::write8(ref<Expr> offset, ref<Expr> value) {
  assert(!isa<ConstantExpr>(offset) &&
...
ObjectState::ObjectState(const MemoryObject *mo, const Array *array, KType *dt)
    : copyOnWriteOwner(0), object(mo), size(array->size), dynamicType(dt),
      readOnly(false) {
  lists.insert({Expr::createTrue(),
                std::make_shared<CachedUList>(mo->parent->getArrayCache(),
                                              array, size, object)});
}

ObjectState::ObjectState(const MemoryObject *mo, KType *dt)
    : copyOnWriteOwner(0), object(mo), size(mo->getSizeExpr()), dynamicType(dt),
      readOnly(false) {
  lists.insert({Expr::createTrue(),
                std::make_shared<CachedUList>(mo->parent->getArrayCache(),
                                              nullptr, size, object)});
...
ObjectState::ObjectState(const ObjectState &os)
    : copyOnWriteOwner(0), object(os.object), size(os.size),
      dynamicType(os.dynamicType), readOnly(os.readOnly),
      wasWritten(os.wasWritten) {
  for (auto other : os.lists) {
    lists.insert({other.first, std::make_shared<CachedUList>(*(other.second))});
  }
}
...
void ObjectState::initializeToZero() {
  ...
  auto array = getArrayCache()->CreateArray(object->getSizeExpr(),
                                            SourceBuilder::constant(values));
  lists.clear();
  lists.insert({Expr::createTrue(), std::make_shared<CachedUList>(
                                        getArrayCache(), array, size, object)});
}

ref<Expr> ObjectState::read8(unsigned offset) const {
  assert(!lists.empty());

  ref<Expr> read = nullptr;
  for (auto i : lists) {
    if (!read) {
      read = i.second->read8(offset);
    } else {
      read = SelectExpr::create(i.first, i.second->read8(offset), read);
    }
  }
...
ref<Expr> ObjectState::read(ref<Expr> offset, Expr::Width width) const {
  ...
  for (auto i : lists) {
    if (!read) {
      read = i.second->read(offset, width);
    } else {
      read = SelectExpr::create(i.first, i.second->read(offset, width), read);
    }
  }
