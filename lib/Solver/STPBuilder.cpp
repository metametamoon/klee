...
namespace {
llvm::cl::opt<bool> UseConstructHash(
...
::VCExpr STPBuilder::buildArray(const char *name, unsigned indexWidth,
                                unsigned valueWidth) {
  // XXX don't rebuild if this stuff cons's
...
ExprHandle STPBuilder::getTrue() { return vc_trueExpr(vc); }
ExprHandle STPBuilder::getFalse() { return vc_falseExpr(vc); }
ExprHandle STPBuilder::bvOne(unsigned width) { return bvZExtConst(width, 1); }
ExprHandle STPBuilder::bvZero(unsigned width) { return bvZExtConst(width, 0); }
...
ExprHandle STPBuilder::bvExtract(ExprHandle expr, unsigned top,
                                 unsigned bottom) {
...
ExprHandle STPBuilder::bvRightShift(ExprHandle expr, unsigned shift) {
  ...
  if (shift == 0) {
    ...
    return vc_bvConcatExpr(vc, bvZero(shift),
...
ExprHandle STPBuilder::bvLeftShift(ExprHandle expr, unsigned shift) {
  ...
  if (shift == 0) {
    ...
    return vc_bvConcatExpr(vc, bvExtract(expr, width - shift - 1, 0),
                           bvZero(shift));
  }
}
...
ExprHandle STPBuilder::constructAShrByConstant(ExprHandle expr, unsigned shift,
                                               ExprHandle isSigned) {
  unsigned width = vc_getBVLength(vc, expr);
  ...
  if (shift == 0) {
    ...
    return vc_iteExpr(
        vc, isSigned,
        ExprHandle(vc_bvConcatExpr(vc, bvMinusOne(shift),
                                   bvExtract(expr, width - 1, shift))),
        bvRightShift(expr, shift));
  }
}

ExprHandle STPBuilder::constructMulByConstant(ExprHandle expr, unsigned width,
                                              uint64_t x) {
  uint64_t add, sub;
  ExprHandle res = 0;
...
ExprHandle STPBuilder::constructUDivByConstant(ExprHandle expr_n,
  ...
  ExprHandle expr_n_64 =
      vc_bvConcatExpr(vc, bvZero(32), expr_n); // extend to 64 bits
  ExprHandle t1_64bits =
...
ExprHandle STPBuilder::constructActual(ref<Expr> e, int *width_out) {
  ...
  case Expr::Constant: {
    ...
    *width_out = CE->getWidth();
    ...
    while (Tmp->getWidth() > 64) {
      ...
      Res = vc_bvConcatExpr(
          vc, bvConst64(Width, Tmp->Extract(0, Width)->getZExtValue()), Res);
    }
    return Res;
  }
  ...
  case Expr::Mul: {
    ...
    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(me->left))
      if (CE->getWidth() <= 64)
        return constructMulByConstant(right, *width_out, CE->getZExtValue());
      ...
      if (CE->getWidth() <= 64) {
        ...
        if (bits64::isPowerOfTwo(divisor)) {
          ...
          if (bits == 0) {
            ...
            return vc_bvConcatExpr(vc, bvZero(*width_out - bits),
        ...
        if (optimizeDivides) {
          if (*width_out == 32) { // only works for 32-bit division
            ExprHandle quotient =
                constructUDivByConstant(left, *width_out, (uint32_t)divisor);
            ExprHandle quot_times_divisor =
                constructMulByConstant(quotient, *width_out, divisor);
            ExprHandle rem =
                vc_bvMinusExpr(vc, *width_out, left, quot_times_divisor);
            return rem;
          }
        }
      }
  ...
  case Expr::Xor: {
    ...
    if (*width_out == 1) {
      ...
      return vc_iteExpr(vc, left, ExprHandle(vc_notExpr(vc, right)), right);
    } else {
