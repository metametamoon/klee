...
// RUN: %klee --write-kqueries --output-dir=%t.klee-out --optimize=false --execution-mode=bidirectional --max-propagations=3 --max-stack-frames=4 --skip-local --skip-global --skip-not-symbolic-objects --initialize-in-join-blocks=true --search=dfs --use-guided-search=none --debug-log=rootpob,backward,conflict,closepob,reached,init %t.bc 2> %t.log
