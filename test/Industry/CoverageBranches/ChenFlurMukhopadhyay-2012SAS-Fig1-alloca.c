...
// RUN: %klee --output-dir=%t.klee-out --track-coverage=branches --optimize=true --use-forked-solver=false --max-memory=6008 --cex-cache-validity-cores --only-output-states-covering-new=true --dump-states-on-halt=all %t1.bc 2>&1 | FileCheck -check-prefix=CHECK %s
