...
// RUN: %klee --output-dir=%t.klee-out --write-kqueries --use-guided-search=error --location-accuracy --mock-external-calls --check-out-of-memory --libc=klee --skip-not-symbolic-objects --skip-local --skip-global --use-lazy-initialization=only --analysis-reproduce=%s.json %t1.bc
