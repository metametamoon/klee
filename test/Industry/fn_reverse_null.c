...
// RUN: %klee --output-dir=%t.klee-out --use-guided-search=error --external-calls=all --mock-external-calls --libc=klee --skip-not-symbolic-objects --skip-local --skip-global --check-out-of-memory --use-lazy-initialization=only --analysis-reproduce=%s.json %t1.bc
