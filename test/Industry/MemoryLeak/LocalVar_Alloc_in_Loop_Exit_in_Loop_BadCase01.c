...
// RUN: %klee --output-dir=%t.klee-out --write-kqueries --max-cycles-before-stuck=0 --use-guided-search=error --check-out-of-memory --mock-external-calls --skip-not-symbolic-objects --skip-local --skip-global --use-lazy-initialization=only --analysis-reproduce=%s.json %t1.bc
