...
// RUN: %klee --output-dir=%t.klee-out --smart-resolve-entry-function --use-guided-search=error --mock-external-calls --libc=klee --skip-not-symbolic-objects --skip-local --skip-global --use-lazy-initialization=only --analysis-reproduce=%s.json %t1.bc
